...
import { useSharedValue, withTiming, runOnJS, Easing, type SharedValue } from 'react-native-reanimated';
...
export function useMapTransform(cfg: TransformConfig): MapTransform {
  ...
  // Keep the map pinned inside the viewport (or centered when smaller than it).
  ...
  // PAN — one finger only. Two-finger drag rides on the pinch focal below, so the
  // two gestures never write tx/ty on the same frame (no jitter).
  ...
  // PINCH — focal-aware. screen = t + s·map  ⇒  keep the point under the fingers
  // fixed while scaling by `ratio`:  t' = focal − (focal − t)·ratio
