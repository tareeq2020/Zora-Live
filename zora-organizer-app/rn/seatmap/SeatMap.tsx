...
// Renderer: react-native-svg (good to ~1,500 nodes). For bigger venues swap in
// SeatMapSkia (see README) — the gesture layer is identical.
...
import type { FloorPlan, Seat } from './types';
import { z } from '../theme';
...
export default function SeatMap({ plan, viewport, onSelect, maxScale = 6 }: Props) {
  ...
  // Hit-test runs on the JS thread (via runOnJS). Inputs are in fitted-content
  // space; divide by `fit` to compare against intrinsic seat coordinates.
  const hitTest = useCallback((mx: number, my: number) => {
    ...
    for (const s of plan.seats) {
      ...
      const touchable = (s.r + 6) ** 2;            // +6 map units of fat-finger forgiveness
  ...
  // Seats are memoised on status only → they render ONCE, never on gesture frames.
  const seatNodes = useMemo(() => plan.seats.map(s => {
  ...
  return (
    <GestureDetector gesture={gesture}>
      <View style={{ width: viewport.width, height: viewport.height, overflow: 'hidden', backgroundColor: z.panel, borderRadius: z.r }}>
        {/* transformOrigin '0 0' makes scale grow from top-left, matching screen = t + s·point. Needs RN >= 0.74. */}
        <Animated.View style={[{ width: contentW, height: contentH, transformOrigin: '0% 0%' }, animStyle]}>
          <Svg width={contentW} height={contentH} viewBox={`0 0 ${plan.width} ${plan.height}`}>
            {plan.stage && (
              <G>
                <Rect x={plan.stage.x} y={plan.stage.y} width={plan.stage.w} height={plan.stage.h}
                      rx={4} fill="#1B1B20" stroke={z.orange} strokeWidth={1.5} />
                <SvgText x={plan.stage.x + plan.stage.w / 2} y={plan.stage.y + plan.stage.h / 2 + 4}
                         fontSize={11} fontWeight="700" fill="#ffab86" textAnchor="middle" letterSpacing={3}>
